...
import { possibleBoardActions, possibleFreeActions, factionsToBePicked } from "./available-command";
...
describe("Available commands", () => {
  describe("factionsToBePicked", () => {
    it("should show all factions at the beginning", () => {
      const engine = new Engine();

      expect(factionsToBePicked(engine)).to.have.members(Object.keys(baseFactions));
    });

    it("should show 2 less factions after one is selected", () => {
      const engine = new Engine();

      expect(factionsToBePicked(engine)).to.include.members([Faction.Gleens, Faction.Xenos]);

      engine.setup.push(Faction.Gleens);

      const factions = factionsToBePicked(engine);
    ...
    describe("when randomFactions is enabled", () => {
      it("should give only one faction at a time", () => {
        ...
        expect(factionsToBePicked(engine)).to.eql([Faction.Bescods]);

        engine.setup.push(Faction.Bescods);

        expect(factionsToBePicked(engine)).to.eql([Faction.Gleens]);

        engine.setup.push(Faction.Gleens);

        expect(factionsToBePicked(engine)).to.eql([Faction.BalTaks]);
      });
      ...
      describe("when auction is enabled", () => {
        it("should offer factions from a randomly selected pool (bid-while-choosing)", () => {
          ...
          expect(factionsToBePicked(engine)).to.have.members([Faction.Bescods, Faction.Gleens, Faction.BalTaks]);

          engine.setup.push(Faction.Gleens);
          expect(factionsToBePicked(engine)).to.have.members([Faction.Bescods, Faction.BalTaks]);

          engine.setup.push(Faction.BalTaks);
          expect(factionsToBePicked(engine)).to.eql([Faction.Bescods]);

          engine.setup.push(Faction.Bescods);
          expect(factionsToBePicked(engine)).to.eql([]);
        });
